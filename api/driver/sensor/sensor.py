...
class SensorReceiver():
    ...
    def receive_data(self):
        ...
        # 데이터 구조를 Set으로 변경
        result_data = {}
        result_data[sensor_label.NAME_TEMP] = float(raw_data[0].strip())
        result_data[sensor_label.NAME_HUMI] = float(raw_data[1].strip())
        result_data[sensor_label.NAME_GAS] = float(raw_data[2].strip())
        result_data[sensor_label.NAME_WATER] = float(raw_data[3].strip())
        result_data[sensor_label.NAME_SONIC] = float(raw_data[4].strip())
        print(result_data)
